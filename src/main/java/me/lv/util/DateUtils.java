...
public class DateUtils {
	...
	public static String getReqMonth() {
		return SHORT_MONTH_FORMAT.format(new Date());
	}

	public static String getReqMonth(Date date) {
	 ...
	 * @return
	 */
	public static List<String> splitDay(Date begin, Date end) {
		List<String> lists = new ArrayList<String>();
		Date day = begin;
		String split = getReqDate(day);
		lists.add(split);
		...
		while (true) {
			...
			split = getReqDate(day);
			if (!lists.contains(split)) {
				lists.add(split);
			}
	 ...
	 * @return
	 */
	/*public static List<String> splitMonth(Date begin, Date end) {
		List<String> lists = new ArrayList<String>();

		Date month = getMonthStart(begin);
		String split = getReqMonth(month);
		lists.add(split);
	...
	/*public static boolean isOverIntervalLimit(String startDateStr, String endDateStr, int interval) {
		...
		try {
			...
			log.error("isOverIntervalLimit() error:", e);
			return false;
		}
	...
	public static long getDateDiff(String startDate, String endDate) {
		...
		try {
			...
			diff = (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) > 0 ? (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) : (date2.getTime() - date1.getTime())
					/ (24 * 60 * 60 * 1000);
		} catch (ParseException e) {
		}
		return diff;
	...
	public static void main(String[] args) throws ParseException {
		...
		String begin = "2017-05-27";
		String end = "2017-07-02";

		/*Date dbegin = DateUtils.parseDate(begin, DATE_FORMAT_DATEONLY);
		Date dend = DateUtils.parseDate(end, DATE_FORMAT_DATEONLY);

		List<String> splits = splitMonth(dbegin, dend);*/
